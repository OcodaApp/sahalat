...
class ProviderOneController extends GetxController {
  ...
  void getAddBasketPackages() async {
    var data  = {
      'provider_id' : providerId.value,
  ...
  void getAddBasketServices(price) async {
    var data  = {
      'provider_id' : providerId.value,
      'type' : provider.type,
      'addres_text' : General.address,
    ...
    Request request = Request(url: urlPostBasketSalon, body: data);
    request.postAuth().then((value) async {
      if (value['status']) {
        Fluttertoast.showToast(
          msg: value['msg'],
