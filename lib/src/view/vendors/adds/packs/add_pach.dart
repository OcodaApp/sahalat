...
class AddPackEvent extends StatelessWidget {
  AddPackEvent({Key? key,this.providerData}) : super(key: key);
  final _formKey = GlobalKey<FormState>();
  var providerData;
  final ImagesController imagescontroller = Get.put(ImagesController(), permanent: true);
  final AddPcakcageController controller = Get.put(AddPcakcageController());
  @override
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          ...
          style: TextStyle(fontSize: sizeW16,fontWeight: FontWeight.w700,color: whitecolor)),
      ),
      body: Container(
        width: Get.width,
        height: Get.height,
            ...
            MaterialButton(
              ...
              onPressed: (){
                if (!_formKey.currentState!.validate()) {
                  ...
                  if(providerData['section']['is_package_image'] == 'yes' && imagescontroller.image.value == ''){
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(content: Text('${'upload'.tr} ${'image'.tr}')),
                    );
                  }else{
                    controller.addPackage(imagescontroller.image.value).then((aaa){
